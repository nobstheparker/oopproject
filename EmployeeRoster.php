...
class EmployeeRoster {
    ...
    public function __construct($rosterSize) {
        ...
        $this->space = $rosterSize; // Initialize space to the size of the roster
    }
    ...
    public function add(Employee $employee) {
        ...
        foreach ($this->roster as $index => $existingEmployee) {
            if ($existingEmployee === null) {
                ...
                $this->space--;  // Decrease available space after adding employee
    ...
    public function remove($empNumber) {
        $index = $empNumber - 1; // Convert 1-based employee number to 0-based index
        if (isset($this->roster[$index]) && $this->roster[$index] !== null) {
            $this->roster[$index] = null; // Set the position to null instead of re-indexing
